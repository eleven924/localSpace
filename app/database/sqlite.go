...
func insertInitialData(db *sql.DB) error {
	...
	configs := []string{
		...
		`INSERT INTO configs (key, value, description) VALUES ('version', '0.3.0', 'Application version') ON CONFLICT(key) DO UPDATE SET value = excluded.value, description = excluded.description`,
	}
