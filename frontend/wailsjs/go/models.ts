export namespace models {
	...
	export class ExitGuardJobSnapshot {
	    id: number;
	    jobType: string;
	    status: string;
	    title: string;
	    ...
	    static createFrom(source: any = {}) {
	        return new ExitGuardJobSnapshot(source);
	    }
	...
	export class ExitGuardSnapshot {
	    ...
	    static createFrom(source: any = {}) {
	        return new ExitGuardSnapshot(source);
	    }
	
	    constructor(source: any = {}) {
	        if ('string' === typeof source) source = JSON.parse(source);
	        this.hasProtectedJobs = source["hasProtectedJobs"];
	        this.total = source["total"];
	        this.statusCounts = source["statusCounts"];
	        this.jobs = this.convertValues(source["jobs"], ExitGuardJobSnapshot);
	    }
